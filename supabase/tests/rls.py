...
def call(method, path, key, body=None, headers=None, bearer=None):
...
user_id = other_id = event_id = None
slug = 'rls-test-' + uuid.uuid4().hex[:8]
email = f'rls-test-{uuid.uuid4().hex[:8]}@example.invalid'
owner_pw = uuid.uuid4().hex
other_email = f'rls-other-{uuid.uuid4().hex[:8]}@example.invalid'
other_pw = uuid.uuid4().hex

try:
    # --- fixtures via service_role -----------------------------------------
    st, u = call('POST', '/auth/v1/admin/users', SVC,
                 {'email': email, 'password': owner_pw, 'email_confirm': True})
    assert st in (200, 201), (st, u)
    user_id = u['id']
    st, tok = call('POST', '/auth/v1/token?grant_type=password', ANON,
                   {'email': email, 'password': owner_pw})
    assert st == 200 and tok.get('access_token'), (st, tok)
    owner_jwt = tok['access_token']

    st, other = call('POST', '/auth/v1/admin/users', SVC,
                     {'email': other_email, 'password': other_pw, 'email_confirm': True})
    assert st in (200, 201), (st, other)
    other_id = other['id']
    st, tok = call('POST', '/auth/v1/token?grant_type=password', ANON,
                   {'email': other_email, 'password': other_pw})
    assert st == 200 and tok.get('access_token'), (st, tok)
    other_jwt = tok['access_token']
    ...
    pid_signed = str(uuid.uuid4())
    st, r = call('POST', '/rest/v1/photos', ANON,
                 {'id': pid_signed, 'event_id': event_id,
                  'storage_path': f'{event_id}/{pid_signed}.jpg',
                  'thumb_path': f'{event_id}/{pid_signed}_thumb.jpg',
                  'width': 4096, 'height': 3072, 'byte_size': 1800000,
                  'mime_type': 'image/jpeg'},
                 bearer=other_jwt)
    check('signed-in non-owner can insert a photo into an open event',
          st in (200, 201), f'{st} {str(r)[:80]}')

    print("\nhost overview:")
    st, r = call('POST', '/rest/v1/rpc/owned_events_with_previews', ANON, {})
    check('anon cannot call host overview', st >= 400, f'{st} {str(r)[:60]}')
    st, r = call('POST', '/rest/v1/rpc/owned_events_with_previews', ANON, {},
                 bearer=other_jwt)
    check('non-owner overview excludes the event', st == 200 and r == [],
          f'{st} {str(r)[:60]}')
    st, r = call('POST', '/rest/v1/rpc/owned_events_with_previews', ANON, {},
                 bearer=owner_jwt)
    overview = next((event for event in r if event['id'] == event_id), None) if st == 200 else None
    expected_thumbs = {
        f'{event_id}/{pid}_thumb.jpg',
        f'{event_id}/{pid_signed}_thumb.jpg',
    }
    check('host overview returns owned event', overview is not None,
          f'{st} {str(r)[:80]}')
    check('host overview aggregates count and thumbnails',
          overview is not None
          and overview['photo_count'] == 2
          and set(overview['previews']) == expected_thumbs,
          str(overview)[:120] if overview else '')
    ...
    check('hidden photo is excluded from event_photos',
          len(r) == 1 and r[0]['id'] == pid_signed, str(r)[:60])
    st, r = call('POST', '/rest/v1/rpc/owned_events_with_previews', ANON, {},
                 bearer=owner_jwt)
    overview = next((event for event in r if event['id'] == event_id), None) if st == 200 else None
    check('overview counts hidden photos but excludes their thumbnail',
          overview is not None
          and overview['photo_count'] == 2
          and overview['previews'] == [f'{event_id}/{pid_signed}_thumb.jpg'],
          str(overview)[:120] if overview else '')
    call('PATCH', f'/rest/v1/photos?id=eq.{pid}', SVC, {'hidden_at': None})
    st, r = call('POST', '/rest/v1/rpc/event_photos', ANON, {'p_event_id': event_id})
    check('unhidden photo reappears', len(r) == 2, str(len(r)))
    ...
    st, r = call('POST', '/rest/v1/rpc/event_photos', ANON, {'p_event_id': event_id})
    check('gallery still readable after close', len(r) == 2, str(len(r)))
    ...
    check('host still sees photos while private', len(r) == 3, f'{len(r)} rows')

    call('PATCH', f'/rest/v1/events?id=eq.{event_id}', SVC, {'gallery_hidden_at': None})
    st, r = call('POST', '/rest/v1/rpc/event_photos', ANON, {'p_event_id': event_id})
    check('undo restores the gallery', len(r) == 3, f'{len(r)} rows')
    st, r = call('POST', '/rest/v1/rpc/event_by_slug', ANON, {'p_slug': slug})
    ...
    if other_id:
        st, _ = call('DELETE', f'/auth/v1/admin/users/{other_id}', SVC)
        print(f"  other user deleted ({st})")
